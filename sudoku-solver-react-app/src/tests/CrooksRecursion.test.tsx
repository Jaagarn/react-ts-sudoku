...
import { recursionMain } from "../solver/CrooksRecursion";
...
describe('testing cooks recursion', () => {
  test('recursionMain solves a simple sudoku 1', () => {
    ...
    expect(recursionMain(board, findCanditates(board))).toStrictEqual(arrayToCells(valid_board_easy_sudoku_solution));
  })
  ...
  test('recursionMain solves a hard sudoku', () => {
    ...
    expect(recursionMain(board, findCanditates(board))).toStrictEqual(arrayToCells(valid_board_hard_sudoku_solution));
  })
  test('recursionMain solves a expert sudoku', () => {
    let newBoard = arrayToCells(valid_board_expert_sudoku);
    let board = findAndFillCanditates(newBoard);
    let solution = recursionMain(board, findCanditates(board));
